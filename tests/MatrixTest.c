...
static void AssertMatricesSameDimensions(Matrix matrix1, Matrix matrix2)
{
	if(Matrix_GetRows(matrix1) != Matrix_GetRows(matrix2))
		TEST_FAIL();
	if(Matrix_GetColumns(matrix1) != Matrix_GetColumns(matrix2))
		TEST_FAIL();
}

static void AssertMatricesEqual(Matrix matrix1, Matrix matrix2)
{
	AssertMatricesSameDimensions(matrix1, matrix2);
	for(int i = 0; i < Matrix_GetRows(matrix1); i++)
	{
		for(int j = 0; j < Matrix_GetColumns(matrix1); j++)
		{
			TEST_ASSERT_EQUAL_FLOAT(Matrix_ValueAt(matrix1, i, j), Matrix_ValueAt(matrix2, i, j));
		}
	}
}
