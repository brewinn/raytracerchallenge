...
static float FillAdd(int i, int j) { return (float)i + j; }

static float FillSubtract(int i, int j) { return (float)i - j; }

static float FillCellNumber(int i, int j) { return (float)4 * i + j; }
...
TEST_SETUP(Matrix) { matrix = Matrix_Create(4, 4); }

TEST_TEAR_DOWN(Matrix) { Matrix_Destroy(&matrix); }
...
static void AssertMatrixTransposed(Matrix matrix1, Matrix matrix2) {
  ...
  for (int i = 0; i < Matrix_GetRows(matrix1); i++) {
    for (int j = 0; j < Matrix_GetColumns(matrix1); j++) {
      TEST_ASSERT_EQUAL_FLOAT(Matrix_ValueAt(matrix1, j, i),
                              Matrix_ValueAt(matrix2, i, j));
    }
  }
}
