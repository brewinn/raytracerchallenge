...
TEST(Intersections, IntersectionsStoresObjectType) {
  ...
  TEST_ASSERT_EQUAL_PTR(NULL, Intersection_GetObject(xs, 0));
  TEST_ASSERT_EQUAL_PTR(sphere, Intersection_GetObject(xs, 1));
}

TEST(Intersections, CanSetAndReturnIntersections) {
  Intersection x = {.time = 1, .object = sphere};
...
TEST(Intersections, HitShouldOffsetThePoint) {
  Ray r = Ray_Create(Tuple_CreatePoint(0, 0, -5), Tuple_CreateVector(0, 0, 1));
...
TEST(Intersection, IntersectionStoresTimeAndType) {
  Intersection x = {.time = 3.5, .object = NULL};
...
TEST(Intersection, AggregateCombinesIntersectionsToIntersections) {
  Intersection x1 = {.time = 1.0, .object = NULL};
  Intersection x2 = {.time = 2.0, .object = NULL};
...
TEST_SETUP(MultipleIntersection) {
  x1 = (Intersection){.time = 5, .object = NULL};
  x2 = (Intersection){.time = 7, .object = NULL};
  x3 = (Intersection){.time = -3, .object = NULL};
  x4 = (Intersection){.time = 2, .object = NULL};
}

TEST_TEAR_DOWN(MultipleIntersection) { Intersection_Destroy(&xs); }
...
TEST_SETUP(Computation) {
  Ray ray =
      Ray_Create(Tuple_CreatePoint(0, 0, -5), Tuple_CreateVector(0, 0, 1));
  sphere = Sphere_Create();
  Intersection x = {4, sphere};
  comp = Intersection_PrepareComputations(x, ray);
}

TEST_TEAR_DOWN(Computation) { Sphere_Destroy(&sphere); }
...
TEST(Computation, InsideTrueWhenIntersectionInside) {
  Ray ray = Ray_Create(Tuple_CreatePoint(0, 0, 0), Tuple_CreateVector(0, 0, 1));
