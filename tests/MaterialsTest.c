...
TEST(MaterialLighting, LightingWithEyeBetweenTheLightAndTheSurface) {
  ...
  Light light = Light_CreatePointLight(Tuple_CreatePoint(0, 0, -10),
                                       Color_Create(1, 1, 1));
  bool inShadow = false;
  Color result =
      Material_Lighting(material, light, position, eyev, normalv, inShadow);
...
TEST(MaterialLighting, LightingWithEyeOppositeSurfaceAndLightOffset45Degrees) {
  Tuple eyev = Tuple_CreateVector(0, 0, -1);
  Tuple normalv = Tuple_CreateVector(0, 0, -1);
  Light light = Light_CreatePointLight(Tuple_CreatePoint(0, 10, -10),
                                       Color_Create(1, 1, 1));
  bool inShadow = false;
  Color result =
      Material_Lighting(material, light, position, eyev, normalv, inShadow);
