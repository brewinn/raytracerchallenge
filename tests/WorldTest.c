...
TEST_TEAR_DOWN(World) { World_Destroy(&world); }
...
TEST(World, ShadeHitGivenAnIntersectionInShadow) {
  world = World_Create();
  pointlight = Light_AllocatePointLight(Tuple_CreatePoint(0, 0, -10),
                                        Color_Create(1, 1, 1));
...
TEST_SETUP(DefaultWorld) { world = World_CreateDefault(); }

TEST_TEAR_DOWN(DefaultWorld) { World_Destroy(&world); }

TEST(DefaultWorld, DefaultWorldLightHasCorrectCharacteristics) {
  Light defaultLight = *World_GetLight(world, 0);
  Light expectedLight = Light_CreatePointLight(Tuple_CreatePoint(-10, 10, -10),
                                               Color_Create(1, 1, 1));
...
TEST(DefaultWorld, ColorWhenRayMisses) {
  Ray ray =
      Ray_Create(Tuple_CreatePoint(0, 0, -5), Tuple_CreateVector(0, 1, 0));
...
TEST(DefaultWorld, ColorWithIntersectionBehindRay) {
  ...
  Ray ray =
      Ray_Create(Tuple_CreatePoint(0, 0, 0.75), Tuple_CreateVector(0, 0, -1));
  actualColor = World_ColorAt(world, ray);
