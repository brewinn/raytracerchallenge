...
Ray Camera_RayForPixel(Camera c, int pixelX, int pixelY) {
  ...
  Tuple pixel =
      Matrix_MultiplyTuple(inverse, Tuple_CreatePoint(worldX, worldY, -1));
  Tuple origin = Matrix_MultiplyTuple(inverse, Tuple_CreatePoint(0, 0, 0));

  Tuple direction = Tuple_Normalize(Tuple_Subtract(pixel, origin));
