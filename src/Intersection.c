...
int Intersection_GetCount(const Intersections xs) { return xs->count; }
...
Intersections Intersection_Combine(Intersections xs1, Intersections xs2) {
  ...
  for (int i = 0; i < xs->count; i++) {
    if (i < count1)
      Intersection_SetIntersection(xs, i, Intersection_GetIntersection(xs1, i));
    else
      Intersection_SetIntersection(
          xs, i, Intersection_GetIntersection(xs2, i - count1));
  }
  Intersection_Destroy(&xs1);
  Intersection_Destroy(&xs2);
  return xs;
...
static void MergeSortIntersections(int i, int j, Intersections xs,
                                   Intersections dummy) {
  if (i >= j)
    return;
  ...
  for (int k = i; k <= j; k++) {
    if (leftSide == middle + 1) {
      Intersection_SetIntersection(dummy, k,
                                   Intersection_GetIntersection(xs, rightSide));
      rightSide++;
    } else if (rightSide == j + 1) {
      Intersection_SetIntersection(dummy, k,
                                   Intersection_GetIntersection(xs, leftSide));
      leftSide++;
    } else if (Intersection_GetTime(xs, leftSide) <
               Intersection_GetTime(xs, rightSide)) {
      Intersection_SetIntersection(dummy, k,
                                   Intersection_GetIntersection(xs, leftSide));
      leftSide++;
    } else {
  ...
  for (int k = i; k <= j; k++)
    Intersection_SetIntersection(xs, k, Intersection_GetIntersection(dummy, k));
}
...
Computation Intersection_PrepareComputations(Intersection x, Ray ray) {
  ...
  comp.overPoint =
      Tuple_Add(comp.point, Tuple_Multiply(comp.normalv, offsetFactor));
  return comp;
}
