...
static Color CalculateDiffuse(Material material, Light light, Tuple point,
                              Tuple normalv) {
  Color effectiveColor = Color_Multiply(material.color, light.intensity);
  Tuple lightv = Tuple_Normalize(Tuple_Subtract(light.position, point));
  float lightDotNormal = Tuple_Dot(lightv, normalv);
  Color diffuse = Color_ScalarMultiply(
      Color_ScalarMultiply(effectiveColor, material.diffuse), lightDotNormal);
  return diffuse;
}

static Color CalculateSpecular(Material material, Light light, Tuple point,
  ...
  Color specular = Color_ScalarMultiply(
      Color_ScalarMultiply(light.intensity, material.specular), factor);
...
Color Material_Lighting(Material material, Light light, Tuple point, Tuple eyev,
  ...
  Color result = Color_Add(Color_Add(ambient, diffuse), specular);
  return result;
}
