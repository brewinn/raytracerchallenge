...
Canvas Canvas_Create(int width, int height) {
  Canvas canvas = {(Color *)calloc(width * height, sizeof(Color)), width,
                   height};
  return canvas;
}

void Canvas_Destroy(Canvas canvas) { free(canvas.pixels); }
...
static void WriteFormatedInteger(FILE *ppm, int colorComponent,
                                 int *lineWidthPointer) {
